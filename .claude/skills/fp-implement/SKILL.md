...
```
1. Find work     → fp tree, fp issue list --status todo, fp search <query>
...
```bash
fp search deploy pipeline            # AND — both words must appear
fp search deploy OR rollback          # OR — either matches
fp search 'kanban -desktop'           # negation — exclude term
fp search "deploy pipeline"           # exact phrase
fp search deploy --format json        # machine-readable output
```
