...
| Chat UI — inline display   | done   | Custom `SubAgentBlock` in the transcript renders per-spawn cards (name, model, live streaming text, final result, tokens in/out + cost footer, open-in-new-tab link, inline red banner on error). Pulled out of the generic tool-activity accordion via a `sub-agents:` group key. |
| Chat UI — open standalone  | done   | `/chat/$threadId` resolves sub-agent threads (fallback lookup beyond the sidebar filter), renders a breadcrumb bar with sub-agent filename + "Parent thread" link. Spawn cards open the sub-agent in a new tab. |
| Chat UI — cost tooltip     | done   | Chat header shows the aggregate (parent + sub-agents) as the main total; hover tooltip shows Parent vs Sub-agents (N) rows plus a per-model breakdown table. |
| `/usage` breakdown         | done   | `TASK_TYPES` includes `sub-agent` and the `/usage` filter auto-lists it via the shared constant, so users can isolate sub-agent spend. |
...
| Sub-agent runner             | done   | Internal `runSubAgent()` helper — Phase 2: now uses `streamText` with an async-generator tool execute; yields throttled snapshots (100 ms tick) so the parent UI streams partial text, status, and cost live. Persists full multi-step conversation (all `steps.response.messages`) to the sub-agent's own thread. | Inline |
...
| Cost aggregation helper      | done   | `src/lib/chat/chat-cost-aggregation.ts` — `getCostBreakdownForThread` + batched `getCostBreakdownsForThreads`. Queries `ChatUsageEvent` so per-model numbers stay accurate even when the parent switches models mid-thread. | Inline |
| Chat header tooltip          | done   | `ChatHeader` accepts a `costBreakdown` prop; the stats row wraps in a shadcn `Tooltip` that renders Parent, Sub-agents (N), and a By-model table. | Inline |
| Inline sub-agent block       | done   | `src/components/assistant-ui/sub-agent-block.tsx` — custom transcript block; per-spawn cards with status glyph, streaming markdown, tokens+cost footer, new-tab link, error banner. | Inline |
| Standalone sub-agent view    | done   | `getChatPageData` now falls back to a direct thread lookup when the id is not in the sidebar list; the route renders a breadcrumb bar with sub-agent filename + parent-thread link. | Inline |
| `/usage` filter update       | done   | `TASK_TYPES` in `chat-usage.ts` extended with `sub-agent`; the `/usage` page already maps over `TASK_TYPES` so the new option appears automatically. | Inline |
| Run history + activity rollups | done | `TaskRunItem`, `WorkflowRunItem`, `TriggerRunItem`, and `ActivityChatThread` gained optional `aggregate*` + `subAgentCount` fields. `RunHistoryTable` + `ChatThreadMeta` prefer aggregate when present and show a `+N` sub-agent badge. | Inline |
...
- 2026-04-18: Phase 3 implementation landed. New `chat-cost-aggregation.ts` helper provides single-thread and batched cost breakdowns (parent vs sub-agents vs per-model). Chat header now displays aggregate as its main total with a hover tooltip showing Parent, Sub-agents (N), and a By-model table. `TaskRunItem` / `WorkflowRunItem` / `TriggerRunItem` / `ActivityChatThread` now carry optional aggregate + `subAgentCount` fields; `RunHistoryTable` and `ChatThreadMeta` prefer aggregate values and show a `+N` sub-agent badge. `/usage` task-type filter picks up `sub-agent` automatically via the shared `TASK_TYPES`. Feature marked done.
- 2026-04-18: Phase 2 implementation landed. `spawn_sub_agents` tool is now an async-generator that yields throttled snapshots (~100ms) of per-spawn state (text, status, tokens, cost); the AI SDK delivers these as preliminary `tool-output-available` chunks so the parent UI streams live. New `SubAgentBlock` transcript component renders per-spawn cards outside the generic tool-activity accordion — status glyph, markdown-rendered streaming text, tokens-in/out + cost footer, open-in-new-tab link, inline red error banner. Standalone sub-agent view works via `/chat/$threadId` with a parent-thread breadcrumb. Phase 3 (cost aggregation across parent+sub-agents with per-model tooltip) still pending.
